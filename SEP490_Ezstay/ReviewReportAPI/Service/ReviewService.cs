...
//     }
//     
//     public async Task<ApiResponse<IEnumerable<ReviewReportResponse>>> GetAll()
//     {
//         var entities = await _reviewRepository.GetAll();
//         var dto = _mapper.Map<IEnumerable<ReviewReportResponse>>(entities);
//
//         if (!dto.Any())
//             return ApiResponse<IEnumerable<ReviewReportResponse>>.Fail("Không có review nào.");
//
//         return ApiResponse<IEnumerable<ReviewReportResponse>>.Success(dto);
//     }
//
//     public async Task<ApiResponse<IEnumerable<ReviewReportResponse>>> GetAllByPostId(Guid postId)
//     {
//         var entities = await _reviewRepository.GetAllByPostId(postId);
//         var dto = _mapper.Map<IEnumerable<ReviewReportResponse>>(entities);
//         if (!dto.Any())
//             return ApiResponse<IEnumerable<ReviewReportResponse>>.Fail("Bài viết chưa có review nào.");
//         return ApiResponse<IEnumerable<ReviewReportResponse>>.Success(dto);
//     }
//
...
//     // }
//     public async Task<ReviewReportResponse> GetByIdAsync(Guid id)
//     {
//         var entity = await _reviewRepository.GetByIdAsync(id);
//         if (entity == null)
//             throw new KeyNotFoundException("ReviewId not found");
//         return _mapper.Map<ReviewReportResponse>(entity);
//     }
//
//     public async Task<ReviewReportResponse?> GetByContractIdAsync(Guid contractId)
//     {
...
//     // }
//     public async Task<ApiResponse<ReviewReportResponse>> AddAsync(Guid userId, Guid contractId, CreateReviewReportRequest request)
//     {
//         var contract = await _contractClientService.GetContractById(contractId);
//         if (contract == null)
//             return ApiResponse<ReviewReportResponse>.Fail("Không tìm thấy hợp đồng.");
//         var existingReview = await _reviewRepository.GetByContractIdAsync(contractId);
//         if (existingReview != null)
//             return ApiResponse<ReviewReportResponse>.Fail("Hợp đồng này đã được review, không thể review thêm.");
//
...
//             return  ApiResponse<ReviewReportResponse>.Fail("K dc qua"+ contract.CheckoutDate.AddMonths(1) +" ngay");
//         var post =  await _postClientService.GetPostIdByRoomIdAsync(contract.RoomId);
//         if (post == null)
//             return ApiResponse<ReviewReportResponse>.Fail("Không tìm thấy bài đăng cho phòng này.");
//         var review = _mapper.Map<ReviewReport>(request);
...
//         review.CreatedAt = DateTime.UtcNow;
//         await _reviewRepository.AddAsync(review);
//         var dto = _mapper.Map<ReviewReportResponse>(review);
//         return ApiResponse<ReviewReportResponse>.Success(dto, "Thêm review thành công");
//     }
//
//     public async Task<ApiResponse<bool>> UpdateAsync(Guid id,Guid userId, UpdateReviewReportDto request)
//     {
//         var entity = await _reviewRepository.GetByIdAsync(id);
//         if (entity == null)
