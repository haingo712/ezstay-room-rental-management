...
using ReviewResponseAPI.Profiles;
using ReviewResponseAPI.Repository;
using ReviewResponseAPI.Repository.Interface;
using ReviewResponseAPI.Service;
...
odatabuilder.EntitySet<ReviewResponseDto>("ReviewResponses");
...
builder.Services.AddControllers().AddOData(options =>
    options.AddRouteComponents("odata", odata)
        ...
        .Expand()
        .Select());
builder.Services.AddAutoMapper(typeof(ReviewResponseProfile).Assembly);
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        options.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "ReviewResponseAPI", Version = "v1" });
